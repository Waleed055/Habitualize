package com.example.habitualize.ui.home.taskdetail
...
import com.example.habitualize.databinding.ActivityTaskDetailBinding
...
@AndroidEntryPoint
class TaskDetailActivity : AppCompatActivity(), TaskDetailAdapter.CallBacks {
    ...
    private fun menuListeners() {
        binding.circleMenuHide.setOnItemClickListener { menuButton ->
            when (menuButton) {
                0 -> {
                    viewModel.hideChallenge(
                        challenge_name,
                        SharePrefHelper.readString(languageCode).toString()
                    )
