...
@HiltViewModel
class TaskDetailViewModel @Inject constructor(val myRepository: MyRepository) : ViewModel() {
    ...
    fun getTasks(context: Context, challenge_name: String, userLang: String) {
        ...
        myRepository.getChallengeDetailLiveListByChallengeName(challenge_name, userLang).observe(context as TaskDetailActivity){
            mTasksList.clear()
            for (item in it) {
                mTasksList.add(
