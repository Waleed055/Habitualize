...
public class BillingDataSource implements LifecycleObserver, PurchasesUpdatedListener,
     ...
     */
    private BillingDataSource(@NonNull Application application, String[] knownSubscriptionSKUs) {
     ...
     */
    public static BillingDataSource getInstance(@NonNull Application application, String[] knownSubscriptionSKUs) {
        if (sInstance == null) {
            synchronized (BillingDataSource.class) {
                if (sInstance == null) {
                    sInstance = new BillingDataSource(application, knownSubscriptionSKUs);
                }
            }
        }
        return sInstance;
    ...
    public void onBillingSetupFinished(BillingResult billingResult) {
        ...
        if (responseCode == BillingClient.BillingResponseCode.OK) {// The billing client is ready. You can query purchases here.
            // This doesn't mean that your app is set up correctly in the console -- it just
            // means that you have a connection to the Billing service.
            reconnectMilliseconds = RECONNECT_TIMER_START_MILLISECONDS;
            billingSetupComplete = true;
            querySkuDetailsAsync();
            checkTimeStampBeforePurchaseQuery();
        } else {
    ...
    public void checkTimeStampBeforePurchaseQuery() {

        try {
            if (SharePrefHelper.Companion.readString("timestamp").equals("")) {
                refreshPurchasesAsync();
            } else {
                long timestamp = Long.parseLong(SharePrefHelper.Companion.readString("timestamp"));
                ...
                boolean isToday = isSameDay(date1, date2);
                if (!isToday) {
                    refreshPurchasesAsync();
                }
            }
    ...
    private void canPurchaseFromSkuDetailsAndPurchaseLiveData(@NonNull MediatorLiveData<Boolean> result, @NonNull LiveData<SkuDetails> skuDetailsLiveData, @NonNull LiveData<SkuState> skuStateLiveData) {
    ...
    public void refreshPurchasesAsync() {
        billingClient.queryPurchasesAsync(BillingClient.SkuType.SUBS, (billingResult, list) -> {
            ...
            if (SharePrefHelper.Companion.readString("timestamp").equals("")) {
                SharePrefHelper.Companion.writeString("timestamp", String.valueOf(System.currentTimeMillis()));
            }
            ...
            if (billingResult.getResponseCode() != BillingClient.BillingResponseCode.OK) {
                ...
                SharePrefHelper.Companion.writeBoolean("is_ad_disable", list.size() > 0);
//                AdsManager.getInstance().setAdsEnabled(list.size() > 0);
                //MopubAdsManager.getInstance().setAdsEnabled(list.size() > 0);
                AdsManager.getInstance().setAdsEnabled(list.size() > 0);
        ...
        if (null != purchases) {
            for (final Purchase purchase : purchases) {
                for (String sku : purchase.getSkus()) {
                    ...
                    if (null == skuStateLiveData) {
                        Log.d(TAG, "Unknown SKU " + sku + ". Check to make " + "sure SKU matches SKUS in the Play developer console.");
                        continue;
                    }
                    updatedSkus.add(sku);
                }
                ...
                if (purchaseState == Purchase.PurchaseState.PURCHASED) {
                    ...
                    if (!purchase.isAcknowledged()) {
                        billingClient.acknowledgePurchase(AcknowledgePurchaseParams.newBuilder().setPurchaseToken(purchase.getPurchaseToken()).build(), billingResult -> {
                            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                                ...
                                Log.d(TAG, "New Purchase Done in Billing Data Source");
                            }
                        });
                    }
                } else {
     ...
     */
    public void launchBillingFlow(Activity activity, @NonNull String sku, String... upgradeSkus) {
        ...
        if (null != skuDetails) {
            if (null != upgradeSkus && upgradeSkus.length > 0) {
                billingClient.queryPurchasesAsync(BillingClient.SkuType.SUBS, (br, purchasesList) -> {
                    ...
                    switch (heldSubscriptions.size()) {
                        case 1:  // Upgrade flow!
                            ...
                            billingFlowParamsBuilder.setSubscriptionUpdateParams(BillingFlowParams.SubscriptionUpdateParams.newBuilder()
                                    .setOldSkuPurchaseToken(heldSubscriptions.get(0).getPurchaseToken())
                                    .build()
                            );
                        ...
                        case 0:
                            ...
                            break;
                        default:
                            Log.d(TAG, heldSubscriptions.size() + " subscriptions subscribed to. Upgrade not possible.");
                    }
                });
            } else {
                BillingFlowParams.Builder billingFlowParamsBuilder = BillingFlowParams.newBuilder();
            ...
            Toast.makeText(activity, "Item details not found!", Toast.LENGTH_SHORT).show();
    ...
    public void checkSubscriptionStatus() {
        billingClient.queryPurchasesAsync(BillingClient.SkuType.SUBS, (billingResult, list) -> {
            if (billingResult.getResponseCode() != BillingClient.BillingResponseCode.OK) {
                ...
                //processPurchaseList(list, knownSubscriptionSKUs);
                if (list.size() > 0) {
                    SharePrefHelper.Companion.writeBoolean("is_ad_disable", true);
//                    AdsManager.getInstance().setEnabledNoAds(true);
                    //MopubAdsManager.getInstance().setAdsEnabled(true);
                    AdsManager.getInstance().setAdsEnabled(true);
                } else {
                    SharePrefHelper.Companion.writeBoolean("is_ad_disable", false);
                    //MopubAdsManager.getInstance().setAdsEnabled(false);
    ...
    public void onPurchasesUpdated(@NonNull BillingResult billingResult, @Nullable List<Purchase> list) {
        switch (billingResult.getResponseCode()) {
            case BillingClient.BillingResponseCode.OK:
                if (null != list) {
                    //Toast.makeText(BaseApplication.getContext(), "BillingResponseCode.OK", Toast.LENGTH_SHORT).show();

                    // Finish Activity Here and Set Ad Status
                    restartApp();
    ...
    private void restartApp() {
        ...
        //MopubAdsManager.getInstance().setAdsEnabled(true);
        AdsManager.getInstance().setAdsEnabled(true);
        currentActivity.startActivity(new Intent(currentActivity, SplashActivity.class));
